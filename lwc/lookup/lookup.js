...
export default class Lookup extends LightningElement {
  ...
  onRecordSelection(event) {
    this.selectedRecordId = event.currentTarget.dataset.sfid;
  ...
  getLookupResult() {
    ...
    console.log('Fields to retrieve-->',this.fields);
    console.log('Additional Condition-->',this.conditions);
    if(this.searchKey.length>=MIN_LENGTH){
      findRecords({ searchKey: this.searchKey, objectName: this.objectApiName, fields: this.fields,conditions: this.conditions})
      .then((result) => {
  ...
  onSeletedRecordUpdate() {
    const passEventr = new CustomEvent('recordselection', {
      detail: { selectedRecordList: this.selectedRecordList,selectedRecordId: this.selectedRecordId,selectedValue: this.selectedValue }
    });
